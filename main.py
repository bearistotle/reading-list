...
# TODO: Reconsider whether full search page is even necessary and rework the
# logic below or delete. Going with not necessary for now. 
# TODO: Get fork separating nav search and main search working. Possibly use a GET for one
# (you don't need a GET handler with no args, since there's no need for a link to the
# search view due to search in navbar, right?)
    """
    ...
    if request.method == "GET":
        test = "GET"
        return render_template("search.html", nav_search_form=nav_search_form, search_form=search_form, test=test)

    if request.method == "POST":

        test = "FAILURE"

        if nav_search_form.search_origin.data == "nav_search":
            # make keyword default search type (if this doesn't work,
            # make keyword default selection in form def)
            if not nav_search_form.search_type.data:

                search_type = "keyword"

            else:

                search_type = nav_search_form.search_type.data
                search_term = nav_search_form.search_term.data
            
            test = "NavBar Search Successful"

        elif search_form.search_origin.data == "main_search":
    ...
    # TODO: make your code capable of handling cases where no isbn-13 exists
    # and where no isbn exists at all
